...
const selectedMap = 'selectedMap'; // the selected map (canvas) changed
const reset = 'reset'; // reset button click
const layout = 'layout'; // layout selection changed
const dataLoaded = 'loaded'; // data finished loading, or was filtered/updated

// map removal: this means the bioMap objects sent in the message, were removed
// by user
const mapRemoved = 'mapRemoved';
const mapAdded = 'mapAdded';
const mapReorder = 'mapReorder';

const featureUpdate = 'featureUpdate'; // change to qtlConfig from modal
 ...
 */
let DrawLazilyMixin = (superclass) => class extends superclass {
...
let RegisterComponentMixin = (superclass) => class extends superclass {
...
class SceneGraphNodeBase {
  ...
  removeChild(node) {
    ...
    let index$$1 = this._children.indexOf(node);
    if (index$$1 > -1) {
 ...
 */
class SceneGraphNodeGroup extends SceneGraphNodeBase {
 ...
 */
class SceneGraphNodeTrack extends SceneGraphNodeBase {
 ...
 */
class FeatureMark extends SceneGraphNodeBase {
...
class MapBackbone extends SceneGraphNodeBase {
...
class FeatureLabel extends SceneGraphNodeBase {
  /**
   * Constructor
   *
   * @param parent - parent scene graph node
 ...
 * the background.
 */
class MapTrack extends SceneGraphNodeTrack {
  ...
  get visible() {
    ...
    for (let i = start; i < stop; i += step) {
      ...
      let item = index$1(this.labelGroup.locMap, this.labelGroup.children[0].globalBounds.left, i, 1)[0];
      if (labels.length === 0) {
        labels.push(item);
        continue;
      }
...
class Dot extends SceneGraphNodeBase {
  /**
   * Constructor
   *
   * @param parent - parent scene graph node
...
class manhattanRuler extends SceneGraphNodeBase {
 ...
 */
class ManhattanPlot extends SceneGraphNodeTrack {
...
class QTL extends SceneGraphNodeBase {
 ...
 * @extends SceneGraphNodeTrack
 */
class QtlTrack extends SceneGraphNodeTrack {
...
class Label extends SceneGraphNodeBase {
  /**
   * Constructor
   *
   * @param parent - parent scene graph node
 ...
 * @extends SceneGraphNodeTrack
 */
class LabelTrack extends SceneGraphNodeTrack {
...
class FeatureTrack extends SceneGraphNodeTrack {
...
class Ruler extends SceneGraphNodeBase {
...
class BioMap extends SceneGraphNodeCanvas {
...
class CorrespondenceMark extends SceneGraphNodeBase {
 ...
 *
 */
class CorrespondenceMap extends SceneGraphNodeCanvas {
 ...
 */
class Popover extends mix(Menu).with(RegisterComponentMixin) {
  ...
  _generateInner(data) {
    ...
    let popover = data.map(item => {
      let start = mithril('div', 'start:  ' + item.model.coordinates.start);
      let stop = mithril('div', 'stop:  ' + item.model.coordinates.stop);
      let tags = item.model.tags.length > 0 && typeof item.model.tags[0] !== 'undefined' ? mithril('div', 'tags:  ', item.model.tags.join('\n')) : [];
      let aliases = item.model.aliases.length > 0 && typeof item.model.aliases[0] !== 'undefined' ? mithril('div', 'aliases:  ', item.model.aliases.join('\n')) : [];
      let links = item.model.source.linkouts.length > 0 ?
        mithril('div', {id: `links-div-${item.model.name}`},
          item.model.source.linkouts.filter(l => (!l.isLinkingService) && item.model.typeLinkedBy(l)).map(
            ...
            item.model.source.linkouts.some(l => {
              ...
              (item.model.links === undefined ? mithril('img[src=images/ajax-loader.gif]') : item.model.links.map(l => {
      ...
      return [mithril(this._buttonTest(item.model), {targetId: item.model.name}),
        mithril('div', {
...
let ColorPicker = {
  ...
  view: function (vnode) {
    // store these bounds, for checking in drawLazily()
    return [mithril('div.color-picker', {style: `display:${vnode.state.hidden[vnode.state.order]}`}, [
      mithril(BaseSelector, {info: vnode.state}),
      mithril(SaturationSelector, {info: vnode.state}),
      mithril(ColorPreview, {info: vnode.state}),
      mithril('div#color-apply-controls', {style: 'text-align:center; margin-left:10px; display:inline-block; padding:auto'},
        [mithril(ColorBox, {info: vnode.state}),//,settings:vnode.attrs.settings}),
          mithril(ColorApplyButton, {info: vnode.state, settings: vnode.state.settings}),
          mithril(ColorResetButton, {info: vnode.state})
        ]
      )
    ])
    ];
...
class FeatureMenu {
  ...
  constructor(data, order) {
    ...
    let modalDiv = {
      ...
      view: function () {
        return mithril('div', {style: 'height:100%; width:100%'}, [
            mithril(CloseButton, {model: model, config: settings, order: order, reset: defaultSettings}),
            mithril(TitleBox, {settings: settings}),
            mithril(TrackMenu, {info: trackConfig, count: 0}),
            mithril('div', {style: 'text-align:center'}, controls)
          ]
        );
      },
      handleGesture: function () {
...
let TitleBox = {
  ...
  view: function (vnode) {
    return mithril('div',{},'Track title: ',
      mithril('input[type=text].title-input', {
...
class HorizontalLayout
  ...
  view() {
   ...
   // });
    return mithril('div.cmap-layout-horizontal',
      [//this.swapComponents,
        this.correspondenceMapComponents.map(mithril),
        this.bioMapOrder.map((i) => {
          return mithril(BioMapComponent, {bioMap: this.bioMapComponents[i]});
        }), this.featureControls,
  ...
  _layoutCorrespondenceMaps() {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      let component = new CorrespondenceMap({
...
class DataSourceModel {
  ...
  deserialize(data) {
    const res = papaparse_min.parse(data, {
      header: true,
      dynamicTyping: true,
      skipEmptyLines: true
    });
...
class ResetButton {
  ...
  view() {
    return mithril('button', {
      ...
      mithril('i.material-icons', 'restore'),
      'Reset View'
    ]);
  }
...
class RemoveMapButton {
  ...
  view(vnode) {
    ...
    return mithril('button', attrs, [
      mithril('i.material-icons', 'remove_circle_outline'),
      'Remove Map'
    ]);
  }
}
...
class AddMapButton {
  ...
  view(vnode) {
    ...
    return mithril('button', attrs, [
      mithril('i.material-icons', 'add_circle_outline'),
      'Add Map'
    ]);
  }
}
...
class ConfigurationButton {
  ...
  view(vnode) {
    ...
    return mithril('button', attrs, [
      mithril('i.material-icons', 'mode_edit'),
      'Configuration'
    ]);
  }
}
...
class UploadButton {
  ...
  view(vnode) {
    ...
    return mithril('button', attrs, [
      mithril('i.material-icons', 'input'),
      'Add Data'
    ]);
  }
}
...
class MapRemovalDialog {
  ...
  view() {
    ...
    return mithril('div.cmap-map-removal-dialog', [
      mithril('h5', plural ? 'Remove Maps' : 'Remove Map'),
      mithril('form', [
        this.model.bioMaps.map(bioMap => {
          return mithril('label.cmap-map-name', [
        ...
        mithril('button.button', {onclick: evt => this._onCancel(evt)}, [
...
class MapAdditionDialog {
  ...
  view() {
    ...
    return mithril('div.cmap-map-addition-dialog', [
      mithril('h5', 'Add Map'),
      mithril('form', [
        mithril('table.u-full-width', [
          mithril('thead',
            mithril('tr', [mithril('th', 'Data Source'), mithril('th', 'Available Maps')])
          ),
          mithril('tbody',
            this.model.sources.map(source => {
              return mithril('tr', [
                mithril('td', source.id),
                mithril('td', allMaps.filter(map => {
                    return (map.source === source &&
                      this.model.bioMaps.indexOf(map) === -1);
                  }).map(map => {
                    return mithril('label', [
                      mithril('input[type="radio"]', {
      ...
      mithril('button', {
          ...
          mithril('i.material-icons', 'keyboard_arrow_left'),
          'Add Map On Left'
        ]
      ),
      mithril('button.button', {
          disabled: !this.selection,
          class: this.selection ? 'button-primary' : 'button',
          onclick: evt => this._onAddRight(evt)
        }, [
          mithril('i.material-icons', 'keyboard_arrow_right'),
          'Add Map On Right'
        ]
      ),
      mithril('button.button', {onclick: evt => this._onCancel(evt)}, [
...
class ConfigurationDialog {
  ...
  view() {
    //const allMaps = this.model.allMaps || [];
    return mithril('div.cmap-map-addition-dialog', [
      mithril('h5', 'Configuration Details'),
      mithril('form', [
        mithril('textarea', {
...
class UploadDialog {
  ...
  view() {
    ...
    return mithril('div.cmap-map-addition-dialog', [
      mithril('h5', 'Add Map'),
      mithril('p', 'Currently only one file may be added at a time. If both a URL and a local file are provided, preference will be given to the local file.'),
      mithril('form', [
        mithril('table.u-full-width', [
          mithril('thead', [
            mithril('tr', [mithril('th', 'URL'), mithril('th', mithril('input[type=text]', {
...
class Tools {
  ...
  view() {
    return mithril('div.cmap-tools', [
      mithril('div.cmap-toolbar.cmap-hbox', [
        mithril(ResetButton),
        //m(FilterButton),
...
class Header {
  ...
  view() {
    return mithril('div.cmap-hbox',
      mithril('h4.cmap-header', [
        'cmap-js ',
        mithril('span.cmap-header', this.appState.header)
      ]));
  }
}
...
class StatusBar {
  ...
  view() {
    return mithril('div', [
      mithril('div.cmap-attribution', this.appState.attribution),
      mithril('div', {id: 'cmap-disclaimer'}, 'cmap-js is still in alpha. As the software is still in development, the current state of the project may not reflect the final release.'),
      mithril('div.cmap-footer', [
        this.appState.busy ? mithril('img[src=images/ajax-loader.gif]') : '',
...
class CircosLayout
   ...
   */
  view() {
    return mithril('div.cmap-layout-circos',
      this.children.map(mithril)
    );
  }
}
 ...
 */
class UI extends mix().with(RegisterComponentMixin) {
  ...
  view() {
    ...
    this._logRenders();
    return mithril('div.cmap-layout.cmap-vbox', [
      mithril(Header, childAttrs),
      mithril(Tools, childAttrs),
      mithril('div#cmap-layout-titles', {style: 'display:inline-flex;'}),
      [mithril('div.cmap-menu-viewport#cmap-menu-viewport', {style: 'display:none;'}),
        mithril('div.cmap-layout-viewport.cmap-hbox', {id: 'cmap-layout-viewport', style: 'position:relative;'},
          mithril(LayoutContainer, {
            appState: this.appState,
  ...
  _setupMousewheel() {
    const hamster$$1 = hamster(this.el);
...
var index$5 = str => encodeURIComponent(str).replace(/[!'()*]/g, x => `%${x.charCodeAt(0).toString(16).toUpperCase()}`);
...
var index$7 = function (encodedURI) {
...
var index$11 = function (obj, predicate) {
	var ret = {};
...
'use strict';
const strictUriEncode = index$5;
const decodeComponent = index$7;
const splitOnFirst = index$9;
const filterObject = index$11;
