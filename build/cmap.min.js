...
var QTL = function (_SceneGraphNodeBase) {
  ...
  function QTL(_ref) {
    ...
    if (initialConfig.labelPosition !== 'none') {
      ...
      if (initialConfig.labelPosition === 'feature') _this.offset += _this.labelSize;
    }
...
var Label = function (_SceneGraphNodeBase) {
  inherits(Label, _SceneGraphNodeBase);
  ...
  function Label(_ref) {
    ...
    var _this = possibleConstructorReturn(this, (Label.__proto__ || Object.getPrototypeOf(Label)).call(this, { parent: parent, tags: [featureModel.name] }));
    ...
    _this.invert = bioMap.view.invert;
    _this.stop = _this.invert ? _this.model.coordinates.start : _this.model.coordinates.stop;
    var y1 = translateScale(_this.stop, _this.view.base, _this.view.visible, _this.invert) * _this.pixelScaleFactor;
    _this.show = false;
    _this.bounds = new Bounds({
      top: y1,
      left: _this.fm.bounds.left,
      width: _this.config.labelSize,
  ...
  createClass(Label, [{
    ...
    value: function draw(ctx) {
      var config = this.config;
      ctx.font = config.labelSize + 'px ' + config.labelFace;
      var y1 = translateScale(this.stop, this.view.base, this.view.visible, this.invert) * this.pixelScaleFactor;
      var width = config.labelSize;
      var left = this.parent.trackPos > 0 ? this.fm.bounds.right : this.fm.bounds.left - width;
      this.bounds = new Bounds({
        top: y1,
        left: left,
        width: width,
 ...
 * @extends SceneGraphNodeTrack
 */
var LabelTrack = function (_SceneGraphNodeTrack) {
  inherits(LabelTrack, _SceneGraphNodeTrack);
  ...
  function LabelTrack(params) {
    classCallCheck(this, LabelTrack);

    var _this = possibleConstructorReturn(this, (LabelTrack.__proto__ || Object.getPrototypeOf(LabelTrack)).call(this, params));

    _this.dirty = true;
    _this.trackPos = _this.parent.trackPos;
    //don't draw labels if they aren't to be shown
    var labelPosition = params.config.labelPosition;
    if (labelPosition === 'none') return possibleConstructorReturn(_this);
    if (labelPosition === 'feature') {
      ...
      _this.qtlMarks = _this.parent.featureData.map(function (model) {
        var fm = new Label({
          featureModel: model,
          parent: _this,
          bioMap: _this.parent.model,
  ...
  createClass(LabelTrack, [{
    ...
    get: function get$$1() {
      ...
      this.children.forEach(function (child) {
        child.show = false;
        var b = child.globalBounds;
        var hits = _this2.locMap.search({
...
var TitleComponent = {
  ...
  _onPan: function _onPan(evt) {
    ...
    var leftSwapBound = leftMap !== null ? this.leftBound - this.bioMaps[leftMap].domBounds.width : null;
    var rightSwapBound = rightMap !== null ? this.leftBound + this.bioMaps[rightMap].domBounds.width : null;
    if (leftMap !== null && selLeftEdge < leftSwapBound) {
      // Swap Left
    ...
    } else if (rightMap !== null && selLeftEdge > rightSwapBound) {
      // Swap Right
...
var HorizontalLayout = function (_mix$with) {
  ...
  createClass(HorizontalLayout, [{
    ...
    value: function oninit(vnode) {
      ...
      pubsub.subscribe(dataLoaded, handler), pubsub.subscribe(mapRemoved, function () {
        _this2.bioMapComponents = [];
        _this2.bioMapOrder = [];
        _this2._onDataLoaded();
        // this.bioMapComponents.forEach(component => component.dirty = true);
        // m.redraw();
      }), pubsub.subscribe(mapAdded, function () {
        _this2.bioMapComponents = [];
        _this2.bioMapOrder = [];
        _this2._onDataLoaded();
    ...
    value: function _layoutBioMaps() {
      ...
      this.bioMapComponents = this.appState.bioMaps.map(function (model, mapIndex) {
        ...
        var component = new BioMap({
          ...
          initialView: _this5.appState.initialView[mapIndex] || model.config
        });
    ...
    value: function _layoutCorrespondenceMaps() {
      ...
      this.correspondenceMapComponents = [];
      for (var i = 0; i < n - 1; i++) {
        var left = this.bioMapComponents[this.bioMapOrder[i]];
        var right = this.bioMapComponents[this.bioMapOrder[i + 1]];
        var layoutBounds = new Bounds({
  ...
  createClass(UI, [{
    ...
    value: function _dispatchGestureEvt(evt) {
      ...
      if (!this.panLock) {
        ...
        //redirect pan to original target component until end of pan
  ...
  createClass(CMAP, [{
    ...
    value: function load(configURL) {
      ...
      mithril.request(configURL).then(function (config) {
        ...
        var viewOverride = index$4.parse(location.search);

        var promises = _this.appState.load(config);
        Promise.all(promises).then(function () {
          if (viewOverride.view.length) {
            var overrideInitialView = [];
            if (typeof viewOverride.view === 'string') {
              viewOverride.view = [viewOverride.view];
            }
            viewOverride.view.forEach(function (view) {
              var filter = _this.appState.allMaps.filter(function (map) {
                return map.name == view;
              });
              if (filter.length) {
                overrideInitialView.push(filter[0]);
              }
            });
            _this.appState.bioMaps = overrideInitialView;
          }
